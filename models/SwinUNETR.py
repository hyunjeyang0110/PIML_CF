...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        self,
        img_size: Sequence[int] | int = (596, 596),
        in_channels: int = 19,
        out_channels: int = 3,
    ...
    def forward(self, x_in):
        ...
        if len(x_in.shape) == 4:  # 2D: [B, C, H, W]
            b, c, h, w = x_in.shape
            original_size = (h, w)
            # Calculate required padding to make it divisible by 32 (2^5 for 5-stage downsampling)
            pad_h = (32 - h % 32) % 32
            pad_w = (32 - w % 32) % 32
            if pad_h > 0 or pad_w > 0:
                # Pad: (pad_left, pad_right, pad_top, pad_bottom)
                x_in = F.pad(x_in, (0, pad_w, 0, pad_h), mode='constant', value=0)
